...
app.MapGet("tasks/done", async (AppDbContext db) => await db.Tasks.Where(task => task.IsDone ?? false).ToListAsync());
...
app.MapPut("/tasks{id}", async (int id, Task inputTask, AppDbContext db) =>
{
    ...
    task.Name = inputTask?.Name ?? task.Name;
    task.IsDone = inputTask?.IsDone ?? task.IsDone;
